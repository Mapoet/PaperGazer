...
from papergazer.core.ingest import ingest_arxiv
...
async def test_arxiv_query(days: int = 5):
    """
    查询近 N 天的 arXiv 论文
    ...
    Args:
        days: 查询天数（默认 5 天）
    """
    console.print(f"[bold green]开始查询近 {days} 天的 arXiv 论文...[/bold green]")
    ...
    try:
        ...
        console.print(f"[green]检查点已设置: {checkpoint}[/green]")
        console.print(f"[cyan]查询分类: {', '.join(config.arxiv.categories)}[/cyan]")

        # 执行 arXiv 巡检
        console.print("[yellow]正在查询 arXiv API...[/yellow]")
        console.print(f"[cyan]最大结果数: {config.arxiv.max_results}[/cyan]")
        console.print(f"[cyan]请求间隔: {config.arxiv.delay_seconds} 秒[/cyan]")
        ...
        # 显示结果
        if items:
            table = Table(title=f"arXiv 论文（近 {days} 天）")
            ...
            table.add_column("更新日期", style="yellow")
            table.add_column("作者", style="blue", max_width=30)
            table.add_column("分类", style="red", max_width=20)

            for item in items:
                title = item.title or "无标题"
                if len(title) > 50:
                    title = title[:47] + "..."
                arxiv_id = item.identifier or "无 ID"
                pub_date = str(item.published_date) if item.published_date else "未知"
                updated_date = str(item.updated_date) if item.updated_date else "未知"
                authors = "N/A"
                ...
                # 提取分类（从identifier或其他字段）
                categories = "N/A"
                # arXiv ID通常包含分类信息，但这里我们简化显示

                table.add_row(title, arxiv_id, pub_date, updated_date, authors, categories)

            console.print(table)
            console.print(f"[green]共显示 {len(items)} 条记录（最多 20 条）[/green]")
        else:
            console.print("[yellow]未找到 arXiv 论文记录[/yellow]")
            console.print("[yellow]可能原因：[/yellow]")
            console.print(f"  1. 近 {days} 天内没有新的 arXiv 论文")
            console.print("  2. API 查询失败或超时")
            console.print("  3. 检查点设置问题")

        # 显示统计信息
        session = get_session()
        total_count = session.query(PaperItem).filter_by(source="arxiv").count()
        session.close()
        console.print(f"[cyan]数据库中 arXiv 论文总数: {total_count}[/cyan]")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_arxiv_query(days))
