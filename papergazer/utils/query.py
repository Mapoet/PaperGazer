"""
多来源论文查询工具
支持 arXiv、Crossref (CNS)、Europe PMC 等数据源的统一查询接口
"""
...
async def query_arxiv_by_days(
    ...
    Returns:
        查询到的论文数量
    """
    logger.info(f"查询近 {days} 天的 arXiv 论文")

    # 设置检查点
    from papergazer.store.db import RunRecord, update_checkpoint
...
async def query_crossref_by_days(
    ...
    """
    查询指定天数内的 Crossref 论文（CNS 期刊）

    Args:
        config: 应用配置
        days: 查询天数
        issns: ISSN 列表，如果为 None 则使用配置中的 CNS ISSN

    Returns:
        查询到的论文数量
    """
    logger.info(f"查询近 {days} 天的 Crossref 论文")
    ...
    # 如果指定了 ISSN，临时修改配置
    if issns:
        original_issns = config.cns.issn.copy()
        # 创建一个临时的 ISSN 配置
        config.cns.issn = {"custom": issns}
        try:
            count = await ingest_crossref(config)
        finally:
            config.cns.issn = original_issns
    else:
        count = await ingest_crossref(config)
...
async def query_europe_pmc_by_days(
    ...
    Returns:
        查询到的论文数量
    """
    logger.info(f"查询近 {days} 天的 Europe PMC 论文")
...
async def query_unpaywall_by_days(
    ...
    Returns:
        统计信息字典
    """
    logger.info(f"查询近 {days} 天论文的 Unpaywall OA 状态")
...
async def query_all_sources_by_days(
    ...
    if "arxiv" in sources:
        try:
            count = await query_arxiv_by_days(config, days)
    ...
    if "crossref" in sources:
        try:
            count = await query_crossref_by_days(config, days)
    ...
    if "eupmc" in sources:
        try:
            count = await query_europe_pmc_by_days(
                config, days, max_results=max_results.get("eupmc", 1000)
            )
    ...
    if "unpaywall" in sources:
        try:
            stats = await query_unpaywall_by_days(
                config, days, limit=max_results.get("unpaywall")
            )
